...
if (isset($_GET['table'])) {
    ...
    try {
        switch ($table) {
            ...
            case 'subscription':
                $sql = "
                    SELECT s.*, p.nama AS package_name, p.harga, p.masa_aktif, sk.nama_sekolah 
                    FROM subscription s
            ...
            case 'data_subscription_sekolah':
                $sql = "
                    SELECT sk.nama_sekolah, p.nama AS package_name, s.valid_until
                    FROM sekolah sk
                    JOIN subscription s ON sk.id_sekolah = s.id_sekolah
                    JOIN package p ON s.id_package = p.id_package
                ";
                $result = fetchAll($sql);
                break;
            ...
            case 'all':
                $result = [
                    ...
                    "devices" => fetchAll("
                        ...
                        JOIN sekolah sk ON d.id_sekolah = sk.id_sekolah
                    "),
                    "package" => fetchAll("SELECT * FROM package"),
                    "subscription" => fetchAll("
                        ...
                        JOIN sekolah sk ON s.id_sekolah = sk.id_sekolah
                    "),
                    "data_subscription_sekolah" => fetchAll("
                        SELECT sk.nama_sekolah, p.nama AS package_name, s.valid_until
                        FROM sekolah sk
                        JOIN subscription s ON sk.id_sekolah = s.id_sekolah
                        JOIN package p ON s.id_package = p.id_package
